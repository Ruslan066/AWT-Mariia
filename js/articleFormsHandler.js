...
export default class articleFormsHandler {
     ...
     */
    assignFormAndArticle(formElementId, cssClass2hideElement, articleId, offset, totalCount, currPage, offsetsmy, articleIdmy) {
        ...
        if (articleId >= 0) { //article edit (update)
            ...
            this.offsetsmy = offsetsmy;
            this.articleIdmy = articleIdmy;
            this.flag = 1;
            this.methodd = 'PUT'
    ...
    assignFormAndArticle2(formElementId, cssClass2hideElement, articleId) {
        this.cssCl2hideElm = cssClass2hideElement;
        const artForm = document.getElementById(formElementId);
        this.formElements = artForm.elements;
    ...
    processArtEditFrmData(event) {
        ...
        if (this.flag === 1) {
            fetch(`${this.serverUrl}/article/${this.articleId}`, postReqSettings)  //now we need the second parameter, an object wih settings of the request.
                ...
                //
                //                                          = `#article/${artIdFromHash}/${offsetFromHash}/${totalCountFromHash}/${currPage}/${offsets}/${articleId}`;

                .finally(() => window.location.hash = `#article/${this.articleId}/${this.offset}/${this.totalCount}/${this.currPage}/${this.offsetsmy}/${this.articleIdmy}`
                )
        } else {
                //for (let i = 0; i < 10; i++) {
                fetch(`${this.serverUrl}/article`, postReqSettings)  //now we need the second parameter, an object wih settings of the request.
                    ...
                    .finally(() =>
                        window.location.hash = "#articles/1/0"
                    )
            //}
        }
    }
